...
from QUEST_EF.preprocess.cardiac_cycle_prediction.model.ResNetAE import ResNetAE
...
class UVT(nn.Module):
    def __init__(self, AE, Transformer, frames_per_video, embedding_dim, SDmode, rm_branch=None, num_hidden_layers=None, cla_last_features=3, reg_ed_only=False):
        super(UVT, self).__init__()
        
        if SDmode == 'cla':
            last_features = cla_last_features
    ...
    def forward(self, x, nB, nF):        
        ...
        if not self.rm_branch == 'SD':
            ...
            if self.SDmode == 'reg':
                ...
                classes_vec = classes_vec.squeeze(-1)
        else:
            classes_vec = None
...
def get_model(emb_dim, img_per_video, SDmode,
    ...
    model_AE = ResNetAE(input_shape=(128, 128, 1), n_ResidualBlock=8, n_levels=4, bottleneck_dim=emb_dim)
    model_AE.decoder = None
    model_AE.fc2 = None
    
    num_berts = num_hidden_layers if use_conv else None
        
    model    = UVT(model_AE, model_T, img_per_video, emb_dim, SDmode, rm_branch=rm_branch, num_hidden_layers=num_berts, cla_last_features=cla_last_features, reg_ed_only=reg_ed_only) 
